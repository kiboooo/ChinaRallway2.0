...
public class ProtectActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    @Bind(R.id.barchart_protect)
    BarChart barchart;
    @Bind(R.id.iv_back)
    ImageView ivBack;
    @Bind(R.id.tv_title_table)
    TextView tvTitleTable;
    @Bind(R.id.protect_search)
    Button ProtectSearch;
    ...
    private final int PROTEC_TSUCCESS = 741;
    private final int PROTECT_FALL = 852;

    private final int PROTECT_CHECK_SUCCESS = 7411;
    private final int PROTECT_CHECK_FALL = 8522;
    ...
    private Handler max_handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case PROTECT_FALL:
                    //出错显示: 错误原因；
                    Toast.makeText(ProtectActivity.this, "获取信息出错，请刷新", Toast.LENGTH_SHORT).show();
                    break;
                case PROTECT_CHECK_SUCCESS:
                    JSONArray Ja = (JSONArray) msg.obj;
                    CheckRecData[] maxCheckRecData = new Gson().fromJson(Ja.toString(), CheckRecData[].class);
                    CheckRecData crd = FindRealData(maxCheckRecData);
                    if (crd != null)
                    initBarChart(crd.getTopRation().floatValue(),
                            crd.getDownRatio().floatValue(),
                            crd.getBottomRatio().floatValue());
                    else
                        Toast.makeText(ProtectActivity.this, "该梁板未经过检测", Toast.LENGTH_SHORT).show();
                    break;
                case PROTECT_CHECK_FALL:
                    Toast.makeText(ProtectActivity.this, "获取信息出错，请确认查询请求", Toast.LENGTH_SHORT).show();
                    break;
            }
        }
    };
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_barchart_protect);
      ...
      /*  if (MaxSpinnerData!= null) {
        ...
        ProtectSearch.setOnClickListener(this);
        tvTitleTable.setText("保护层厚度统计");

    }

    private CheckRecData FindRealData(CheckRecData[] checkRecDatas) {
    ...
    /*private void BoundSpinner() {
       *//* ArrayAdapter<String> mAdapter = new ArrayAdapter<>(this, R.layout.spinner_item, MaxSpinnerData);
        //绑定 Adapter到控件
        ProectSpinner.setAdapter(mAdapter);
        ProectSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
    ...
    private void initBarChart(final float TopBeam,final float WebBeam,final float floorBeam) {
        ...
        xVals.add("");
        yVals.add(new BarEntry(TopBeam, 0));
        yVals2.add(new BarEntry(WebBeam, 0));
        yVals3.add(new BarEntry(floorBeam, 0));


        BarDataSet barDataSet = new BarDataSet(yVals, "顶板");
        barDataSet.setColor(Color.rgb(205, 205, 205));//设置第一组数据颜色

        BarDataSet barDataSet2 = new BarDataSet(yVals2, "腹板");
        barDataSet2.setColor(Color.rgb(114, 188, 223));//设置第二组数据颜色

        BarDataSet barDataSet3 = new BarDataSet(yVals3, "底板");
        barDataSet3.setColor(Color.rgb(255, 123, 124));//设置第三组数据颜色
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.iv_back:
                finish();
                break;
            case R.id.protect_search:
                if (!BeamRequestContent.equals("")) {
                    String SearchParam =
                            BeamRequestContent.substring(0,BeamRequestContent.indexOf("_"));
